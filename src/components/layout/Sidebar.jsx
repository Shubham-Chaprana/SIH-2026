...
export default function Sidebar() {
  ...
  return (
    <nav className="flex flex-col items-center w-[74px] min-h-0 bg-white/95 border-r border-[var(--color-border)] py-3 gap-1.5 shrink-0 backdrop-blur-sm">
      {NAV_ITEMS.map((item) => {
        ...
        return (
          <NavLink
            ...
            className={`
              ...
              ${isActive
                ? 'bg-[var(--color-accent-subtle)] text-[var(--color-text-primary)] shadow-[inset_0_0_0_1px_rgba(245,197,24,0.18)]'
          ...
          >
            ...
            <span className="mt-0.5 text-[12px] font-medium leading-tight">{item.label}</span>
          </NavLink>
        );
      })}
    </nav>
...
/* --- Icon components — simple SVG, no library dependency --- */
